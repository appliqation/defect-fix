...
describe('CodingTools', () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'defect-fix-test-'));
  ...
  describe('write_file / read_file', () => {
    it('writes a file and reads it back', async () => {
      const writeResult = await tools.dispatch('write_file', { path: 'src/login.ts', content: 'export const x = 1;' });
      expect(writeResult.ok).toBe(true);
      expect(existsSync(join(dir, 'src/login.ts'))).toBe(true);

      const readResult = await tools.dispatch('read_file', { path: 'src/login.ts' });
      expect(readResult.ok).toBe(true);
      expect(readResult.text).toBe('export const x = 1;');
    });

    it('records the written path with a timestamp', async () => {
      await tools.dispatch('write_file', { path: 'src/login.ts', content: 'x' });
      const written = tools.getWrittenPaths();
      expect(written.has('src/login.ts')).toBe(true);
      expect(written.get('src/login.ts')).toBeGreaterThan(0);
    });
    ...
    describe('symlinks — the lexical check above can\'t catch these', () => {
      ...
      beforeEach(() => {
        outsideDir = mkdtempSync(join(tmpdir(), 'defect-fix-outside-'));
  ...
  describe('run_command — allowed command execution and tracking', () => {
    ...
    it('lastPlaywrightTestRun returns the most recent playwright test invocation, real outcome included', async () => {
      ...
      await tools.dispatch('run_command', { command: 'npx', args: ['playwright', 'test', '--', '--appq-run-id=run_1', '--grep', 'Phone Number'] });
    ...
    it('does not leak the parent process env to the child — only the safe allowlist passes through', async () => {
      process.env.APPQ_API_KEY = 'sk-super-secret';
      mockExecSuccess('ok');
      await tools.dispatch('run_command', { command: 'node', args: ['--version'] });
