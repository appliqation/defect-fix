...
// a gated project's spec (or this agent's own Phase 5 --appq-run-id
// reporter) just can't authenticate, and that environmental failure gets
// reported as a real product failure, exactly what this family's
// verification discipline exists to prevent. This list is deliberately
// explicit — see automation-sdk-js's README/.env.example for the
// vocabulary — not a blanket APPQ_/APPLIQATION_ prefix passthrough, which
// would also leak this agent's own APPQ_API_KEY.
const SAFE_ENV_VARS = [
  'PATH',
  'HOME',
  'LANG',
